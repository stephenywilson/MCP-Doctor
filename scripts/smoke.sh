...
TOOL_CALLS_DIR="examples/tool-calls"
...
check "policy file is valid JSON" "python3 -m json.tool '$POLICY_TMP'"
...
check ".env write action is BLOCK"     "echo '$ENV_OUT' | grep -q 'BLOCK'"

READ_OUT=$(node dist/index.js firewall audit --file "$TOOL_CALLS_DIR/safe-read-file.json" --out "$OUT_DIR/fw-read" 2>&1)
check "safe read classified LOW"  "echo '$READ_OUT' | grep -q 'LOW'"
check "safe read action is ALLOW" "echo '$READ_OUT' | grep -q 'ALLOW'"

SHELL_OUT=$(node dist/index.js firewall audit --file "$TOOL_CALLS_DIR/dangerous-shell-command.json" --out "$OUT_DIR/fw-shell" 2>&1)
check "dangerous shell is HIGH or CRITICAL" "echo '$SHELL_OUT' | grep -qE 'HIGH|CRITICAL'"
